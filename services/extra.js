...
// ─── Baby Card (one entry inside the repeatable list) ─────────────────────────
const BLANK_BABY = () => ({
  first_name: '',
  last_name: '',
  dob: '',
...
const BabyCard = ({ index, baby, onChange, onRemove, canRemove }) => {
  const set = (key) => (val) => onChange(index, { ...baby, [key]: val });

  return (
    <View className="bg-slate-50 border border-slate-100 rounded-3xl p-4 mb-4">
      <View className="flex-row items-center justify-between mb-3">
        <View className="flex-row items-center">
          <View className="bg-purple-100 w-7 h-7 rounded-full items-center justify-center mr-2">
            <Text className="text-purple-700 font-bold text-xs">{index + 1}</Text>
          </View>
        ...
        {canRemove && (
          ...
          >
            ...
            <Text className="text-red-500 font-bold text-[10px] ml-1 uppercase tracking-wider">Remove</Text>
          </TouchableOpacity>
        )}
      </View>
      ...
      <View className="flex-row gap-3">
        <View className="flex-1">
          <InputField label="First Name" placeholder="Required" required value={baby.first_name} onChangeText={set('first_name')} />
        </View>
        <View className="flex-1">
          <InputField label="Last Name" placeholder="Required" required value={baby.last_name} onChangeText={set('last_name')} />
        </View>
      </View>

      <DateField label="Date of Birth" value={baby.dob} onChange={set('dob')} required type="dob" />

      <View className="flex-row gap-3">
        <View className="flex-1">
          <InputField label="Birth Weight (kg)" placeholder="e.g. 3.2" keyboardType="numeric" value={baby.birth_weight_kg} onChangeText={set('birth_weight_kg')} />
        </View>
        <View className="flex-1">
          <ToggleField
            label="Gender"
...
export default function RegisterLactatingMother() {
  ...
  const [babies, setBabies] = useState([BLANK_BABY()]);
  ...
  // ── Baby list handlers ──────────────────────────────────────────────────────
  const handleBabyChange = (index, updatedBaby) => {
    setBabies((prev) => prev.map((b, i) => (i === index ? updatedBaby : b)));
  };

  const handleAddBaby = () => {
    setBabies((prev) => [...prev, BLANK_BABY()]);
  };
  ...
  const handleSave = async () => {
    const required = ['first_name', 'last_name', 'dob', 'phone_number'];
    const missing  = required.filter((k) => !formData[k]);

    const babyMissing = babies.some((b) => !b.first_name || !b.last_name || !b.dob);

    if (missing.length > 0 || babyMissing) {
    ...
    // number_of_babies is derived — no longer a manually typed field
    body.append('number_of_babies', String(babies.length));

    // Nested array convention: babies[0][first_name], babies[0][dob], etc.
    // NOTE: confirm this matches what the backend expects for array fields
    // in multipart/form-data — some APIs expect a JSON string instead.
    babies.forEach((baby, index) => {
      Object.entries(baby).forEach(([key, value]) => {
        if (value === '' || value === null || value === undefined) return;
        body.append(`babies[${index}][${key}]`, String(value));
      });
    });
...
console.log('Submitting lactating mother data:', body);
    try {
    ...
    <SafeAreaView className="flex-1 bg-white" edges={['top']}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1">
        ...
        <ScrollView className="flex-1 px-6 pt-6" showsVerticalScrollIndicator={false}>
          ...
          <DateField label="Date of Birth" value={formData.dob} onChange={set('dob')} required type="dob" />
          ...
          {/* ── Baby's Details (repeatable) ── */}
          <Divider />
          <View className="flex-row items-center justify-between mb-4">
            <SectionTitle color="text-purple-600">
              Baby's Details {babies.length > 1 ? `(${babies.length})` : ''}
            </SectionTitle>
          </View>
          ...
          <TouchableOpacity
            ...
            className="flex-row items-center justify-center border-2 border-dashed border-purple-200 rounded-2xl py-3.5 mb-6 bg-purple-50/50"
          >
            <Feather name="plus-circle" size={16} color="#7C3AED" />
            <Text className="text-purple-600 font-bold text-xs ml-2 uppercase tracking-wider">
              Add Another Baby
            </Text>
          </TouchableOpacity>
